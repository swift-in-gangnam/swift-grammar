...
- 지정 이니셜라이저 : 부모클래스의 이니셜라이저 호출 가능, 클래스의 모든 프로퍼티 초기화해야 함, 클래스에 하나 이상 정의해야함, 부모클래스의 지정 이니셜라이저가 역할 충분히 하면 자식클래스는 없어도 됨
- 편의 이니셜라이저 : 지정 이니셜라이저를 내부에서 호출
...
class Student: Person{
  ...
  convenience init(name: String){ //편의이니셜라이저 -> 안붙임
